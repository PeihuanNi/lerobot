...
class SmolVLMEncoderLayer(GradientCheckpointingLayer):
    ...
    def forward_partial(
        ...
        queries = queries[:, :, update_indices, :]
...
class SmolVLMEncoder(nn.Module):
    ...
    def forward(
        ...
        if update_mask is None or cache is None:
            ...
            return (output, layer_outputs) if return_layer_outputs else output
        ...
        for layer_idx, encoder_layer in enumerate(self.layers):
            cached_output = cache["layer_outputs"][layer_idx]
            hidden_states = encoder_layer.forward_partial(
                hidden_states,
                attention_mask,
                update_indices,
...
class SmolVLMVisionTransformer(SmolVLMPreTrainedModel):
    ...
    def forward(
        ...
        **kwargs: Unpack[TransformersKwargs],
        ...
        _ = kwargs.pop("full_update_interval", None)
        ...
        if use_partial_update:
            ...
            if not same_mask:
                ...
                if update_indices.numel() == patch_attention_mask.shape[1]:
                    ...
                    update_indices = update_indices.to(hidden_states.device)
        ...
        if use_partial_update:
            ...
            cache_valid = (
                cache is not None
                and cache.get("embeddings") is not None
                and cache["embeddings"].shape == hidden_states.shape
                and cache["embeddings"].device == hidden_states.device
                and len(cache.get("layer_outputs", [])) == len(self.encoder.layers)
                and cache["layer_outputs"][0].shape == hidden_states.shape
            )
        else:
            cache_valid = False
        ...
        full_update = force_full_update or not use_partial_update or not cache_valid
        if full_update:
            encoder_result = self.encoder(
                inputs_embeds=hidden_states,
                attention_mask=attention_mask_full,
                return_layer_outputs=use_partial_update,
            )
            if use_partial_update:
                encoder_outputs, layer_outputs = encoder_result
                self._partial_update_cache[cache_key] = {
                    "embeddings": hidden_states.detach(),
            ...
            hidden_states = merge_with_cache(hidden_states, cache["embeddings"], update_mask, use_ste)
            if use_ste:
                encoder_outputs, layer_outputs = self.encoder(
                    inputs_embeds=hidden_states,
                    attention_mask=attention_mask_full,
                ...
                attention_mask_partial = _prepare_4d_attention_mask(patch_attention_mask, hidden_states.dtype)
                if attention_mask_partial is not None:
                    attention_mask_updated = attention_mask_partial[:, :, update_indices, :]
                encoder_outputs, layer_outputs = self.encoder(
                    inputs_embeds=hidden_states,
