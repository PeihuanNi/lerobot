...
def rollout_with_viz(
    ...
    while not np.all(done) and step < max_steps:
        ...
        for key in observation:
            if 'pixel' in key.lower() or 'image' in key.lower() or 'rgb' in key.lower():
                img = observation[key]
                if step == 0:
                    print(f"[DEBUG] Found image key: {key}, type: {type(img)}, shape: {getattr(img, 'shape', 'N/A')}")
                if isinstance(img, np.ndarray):
                    ...
                    break
        if step < 3:
            print(f"[DEBUG] step={step}, prev_image: {prev_image is not None}, curr_image: {curr_image is not None}")
        # 保存当前图像
        curr_image = None
        for key in observation:
            if 'pixel' in key.lower() or 'image' in key.lower() or 'rgb' in key.lower():
        ...
        #     obs_after_preprocess = preprocess_observation(observation)
        #     print(f"[DEBUG] after preprocess_observation: {list(obs_after_preprocess.keys())}")
